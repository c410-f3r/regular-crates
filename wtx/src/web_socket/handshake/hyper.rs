  ...
  S: AsyncRead + AsyncWrite + Send + Unpin + 'static,
{
  ...
  async fn handshake(
    ...
    drop(self.req.headers_mut().insert(CONNECTION, HeaderValue::from_static("upgrade")));
    drop(self.req.headers_mut().insert(HOST, host));
    drop(self.req.headers_mut().insert("Sec-WebSocket-Key", gen_key(&mut <_>::default()).parse()?));
    drop(self.req.headers_mut().insert("Sec-WebSocket-Version", HeaderValue::from_static("13")));
    drop(self.req.headers_mut().insert(UPGRADE, HeaderValue::from_static("websocket")));
  ...
  T: AsyncBounds,
{
  ...
  fn upgrade(self) -> crate::Result<(Self::Response, Self::Upgrade)> {
    ...
    let swk = sws_opt.ok_or(MissingHeader { expected: ExpectedHeader::SecWebSocketKey })?;
    if self.req.headers().get("Sec-WebSocket-Version").map(HeaderValue::as_bytes) != Some(b"13") {
      return Err(MissingHeader { expected: ExpectedHeader::SecWebSocketVersion_13 });
    }
    let res = Response::builder()
      .status(StatusCode::SWITCHING_PROTOCOLS)
      .header(CONNECTION, "upgrade")
      .header(UPGRADE, "websocket")
      .header("Sec-WebSocket-Accept", derived_key(&mut <_>::default(), swk.as_bytes()))
      .body(Body::from("switching to websocket protocol"))?;
    let stream = UpgradeFutHyper { inner: upgrade::on(self.req) };
    Ok((res, stream))
  }
}
...
fn verify_headers(hm: &HeaderMap) -> crate::Result<()> {
  ...
  {
    return Err(MissingHeader { expected: ExpectedHeader::Upgrade_WebSocket });
  }
  if !hm
    .get("Connection")
    .map(|h| h.as_bytes())
  ...
  {
    return Err(MissingHeader { expected: ExpectedHeader::Connection_Upgrade });
  }
  Ok(())
}
